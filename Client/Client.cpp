...
int main()
{
	...
	if (client.Connect(_T("192.168.1.62"), PORT) != 0) {
		...
		printf("Enter your nickname (less than 100 characters): ");
		fflush(stdout);
		fgets(nickname, 100, stdin);
		nickname = standard(nickname);
		int lenName = strlen(nickname);
		client.Send((char*)&lenName, sizeof(int), 0);
		client.Send((char*)nickname, lenName, 0);
		...
		while (true) {
			...
			int lenCompress;
			char compress[10] = { 0 };
			client.Receive((char*)&lenCompress, sizeof(int), 0);
			client.Receive((char*)compress, lenCompress, 0);
			compress[lenCompress] = '\0';
		...
		while (running) {
			// reading command
			printf("Press ctrl + c to quit or type install to download files: ");
			fflush(stdout);
			while (true) {
			...
			// invalid command
				...
				printf("Invalid command. Please type again in new line!\n");
